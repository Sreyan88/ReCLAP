...
class CsvDataset(Dataset):
    def __init__(self, input_filename, img_key, caption_key, sep="\t", is_train = True):
        logging.debug(f"Loading csv data from {input_filename}.")
        df = pd.read_csv(input_filename, sep=sep)

        self.images = df[img_key].tolist()
        self.captions = df[caption_key].tolist()
        self.is_train = True if is_train else False
        if self.is_train:
            self.rewritten_captions = df['rewritten_captions'].fillna('Missing').tolist()
        self.dataset_names = df['dataset']
        self.split_column = None
        if not self.is_train:
            self.split_column = df["split_name"]
    ...
    def __getitem__(self, idx):
        try:
            audio = torchaudio.load(self.images[idx])
        except:
            print(f"Error loading the file: {self.images[idx]}")
            audio = (torch.zeros([1,160000]), 16000)
        text = str(self.captions[idx])
        name = str(self.dataset_names[idx])

        if self.is_train:
            present = False
            if self.rewritten_captions[idx] != 'Missing' and len(eval(self.rewritten_captions[idx])) != 0 :
                rewritten_captions = eval(self.rewritten_captions[idx])
                present = True

            if present:
                try:
                    rewritten_caption = random.choice(rewritten_captions)
                    text = random.choices([text, rewritten_caption], weights=[40, 60], k=1)[0]
                except:
                    print(rewritten_captions)
        ...
        if self.is_train:
            ...
            is_train = str(self.split_column[idx])

        data_dict = {
            "audio": audio,
            "raw_text": text,
            "__url__": name + "-" +  is_train
...
def get_csv_dataset(args, model_cfg, is_train, max_len=480000):
    ...
    dataloader = DataLoader(
        ...
        sampler=sampler,
        drop_last=is_train,
        collate_fn = partial(collate_fn_with_preprocess,max_len=max_len,
...
def preprocess_single(
    ...
    """
    audio_data, orig_sr = sample["audio"]
    audio_data = torchaudio.functional.resample(audio_data, orig_freq = orig_sr, new_freq = 48000)
    ...
    if "[" in sample["raw_text"] and ("]" in sample["raw_text"]):
        texts = eval(sample["raw_text"])
    else:
        texts = sample["raw_text"]
    ...
    sample["text"] = tokenizer(sample["raw_text"], tmodel=tmodel)
...
def collate_fn_with_preprocess(batch,
    ...
    for sample in batch:
        data_preprocessed.append(
            preprocess_single(sample, max_len, audio_cfg, tmodel, class_index_dict, data_filling,
                              data_truncating))
    ...
    for k in data_preprocessed[0].keys():
        if isinstance(data_preprocessed[0][k], dict):  # dealwith bert tokenizer output
            ...
            for kk in data_preprocessed[0][k].keys():
                ...
                # print('batch_dict[k][kk]:', batch_dict[k][kk].shape)
...
def get_data(args, model_cfg):
    ...
    if args.dataset_type == "webdataset":
        ...
        val_dataset_names = ['Clotho', 'audiocaps'] \
