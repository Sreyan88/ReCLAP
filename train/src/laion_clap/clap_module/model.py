...
import math
from clap_module import lorentz as L
...
from .htsat import create_htsat_model, create_beats_model
from transformers import BertModel, RobertaModel, BartModel, T5EncoderModel
...
class CLAP(nn.Module):
    def __init__(
        ...
        if audio_cfg.model_type == "PANN":
            ...
            self.audio_branch = create_beats_model(audio_cfg)
        else:
    ...
    def device(self) -> torch.device:
        return self.logit_scale_a.device
    ...
    def encode_audio(self, audio, device):
        if self.audio_cfg.model_type == "HTSAT":
            ...
            # audio_feats = audio_feats * self.visual_alpha.exp()
            # with torch.autocast(self.device.type, dtype=torch.float32):
            #     audio_feats = L.exp_map0(audio_feats, self.curv.exp())
            # return audio_feats
        elif self.audio_cfg.model_type == "BEATS":
            return self.audio_branch.extract_features(audio, mixup_lambda=None, device=device)
            # audio_feats = audio_feats * self.visual_alpha.exp()
            # with torch.autocast(self.device.type, dtype=torch.float32):
            #     audio_feats = L.exp_map0(audio_feats, self.curv.exp())
            # return audio_feats

            # , mixup_lambda=None, device=device
